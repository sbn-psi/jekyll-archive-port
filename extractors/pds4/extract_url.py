...
def main(argv=None):
    ...
    with urllib.request.urlopen(url) as f:
        s = BeautifulSoup(f.read(), 'lxml-xml')
...
def convert(s):
    if s.Product_Bundle:
        return bundle.extract_label(s)
    if s.Product_Collection:
        return collection.extract_label(s)
    if s.Product_Observational:
        return product.extract_label(s)
